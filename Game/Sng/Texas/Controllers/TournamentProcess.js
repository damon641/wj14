...
module.exports = {
    ...
    joinTournament: async function (data) {
		try {
			...
			let tournament = await Sys.Game.Sng.Texas.Services.sngTournamentServices.getById(data.tournamentId);
			if (!tournament) {
				return {	status: 'fail',	result: null,message: "Tournament not found",	statusCode: 401	};
            }
            let playerRoom = null;
            for(let i=0;i<tournament.rooms.length;i++){
                ...
                let room = await Sys.Game.Sng.Texas.Services.RoomServices.get(tournament.rooms[i]);
    ...
    startBlindTimer : async function(tournament,newRoomIds){
        ...
        Sys.Timers[tournament.id] = setInterval(async function(){
            ...
            for(let i = 0; i < newRoomIds.length;i++){
                let room = await Sys.Game.Sng.Texas.Services.RoomServices.get(newRoomIds[i]);
                ...
                await Sys.Io.of(Sys.Config.Namespace.CashSngTexas).to(newRoomIds[i]).emit('OnBlindLevels', {
                  roomId: newRoomIds[i],
                  current : {
            ...
            if(minutes >= parseFloat(Sys.Tournaments[tournament.id].blind_levels_rise_time)){
                ...
                let room;
                for(let i=0;i<newRoomIds.length;i++){
                  room = await Sys.Game.Sng.Texas.Services.RoomServices.get(newRoomIds[i]);
                  room.currentBlindIndex++;
                    ...
                    await Sys.Io.of(Sys.Config.Namespace.CashSngTexas).to(newRoomIds[i]).emit('OnBlindLevelsRaised', {
                        tournamentId: tournament.id,
                        type : 'reguler',
