...
class Room {
  constructor(id, tableType, currencyType, name, smallBlind, bigBlind,dealerIndex,smallBlindIndex,bigBlindIndex, minPlayers, maxPlayers, minBuyIn, maxBuyIn, type, gameLimit, status, dealer, players, gameWinners, gameLosers, turnBet, game, currentPlayer, rackPercent, rackAmount, expireTime, owner, tableNumber,timerStart,limit,gameType,isCashGame,otherData,isTournamentTable,tournamentType,tournament, lastFoldedPlayerIdArray, currentBlindIndex,betCounter,allInInLimit) {
    ...
    this.players = [];
      if (players && Array.isArray(players)) {
        ...
        players.forEach(function (player) {
          ...
          room.players.push(new Sys.Game.Sng.Omaha.Entities.Player().createObject(player));
    ...
    if (gameLosers) {
      gameLosers.forEach(function (player) {
       room.gameLosers.push(new Sys.Game.Sng.Omaha.Entities.Player().createObject(player));
    ...
    if (game) {
      ...
      this.game = new Sys.Game.Sng.Omaha.Entities.Game().createObject(game);
  ...
  AddPlayer(id, socketId, playerName, avatar, fb_avatar, chips, seatIndex, autoBuyin, isIdeal) {
    ...
    if (chips >= this.minBuyIn) {
      let player = new Sys.Game.Sng.Omaha.Entities.Player(id, socketId,seatIndex, playerName, avatar, fb_avatar, "Waiting", chips,0,chips,false,false,false,[],autoBuyin,0,false,false,false,false,false,null, false, 0, false, false, false, false, isIdeal, 0, false);
     // console.log("Player -> :",player)
      ...
      Sys.Game.Sng.Omaha.Services.playerGameHistoryServices.create(playerGameObj);
  ...
  async StartGame() {
    try {
     ...
     if(room.players[room.bigBlindIndex].sitOutNextBigBlind == true){
      ...
      room.players[room.bigBlindIndex].status = 'Ideal';
       //await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('PlayerLeft', { 'playerId':  room.players[room.bigBlindIndex].id });
      await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('onIdealPlayer', { 'playerId': room.players[room.bigBlindIndex].id,status : true, roomId: room.id });
    ...
    if (room.game == null && playersLength >= this.minPlayers) {
      ...
      let game = await Sys.Game.Sng.Omaha.Services.GameServices.create(gameobj);
      ...
      console.log("----------------------------------------------------------------------");
        ...
        if (playersLength >= room.minPlayers) {
           room.game.status = 'Running';
           room.status = 'Running';
           let gameStarted = await Sys.Game.Sng.Omaha.Controllers.RoomProcess.newGameStarted(room);
           room.initNewRound();
        }
  ...
  async initNewRound() {
    ...
    room.game.deck = [];
    await new Sys.Game.Sng.Omaha.Entities.Deck().fillDeck(room.game.deck);
    room.NewRound();
  };
  ...
  async NewRound() {
    ...
    // get currentPlayer
     ...
     let newRoundStarted = Sys.Game.Sng.Omaha.Controllers.RoomProcess.newRoundStarted(room);
