...
module.exports = {
	...
	subscribeRoom: async function (socket, data){
		try {
			...
			var player = await Sys.Game.Sng.Omaha.Services.PlayerServices.getById(data.playerId);
			let room = await Sys.Game.Sng.Omaha.Controllers.RoomProcess.checkRoomSeatAvilability(socket,data);
			...
			if(room.game){
				...
				await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to([socket.id]).emit('OnSubscibePlayersCards', { playersCards : playersCards, roomId:room.id });
				...
				for(let i=0; i < room.players.length; i++){
					...
					if(room.players[i].id == data.playerId && room.players[i].status == 'Playing' && room.players[i].cards.length == 4 && room.players[i].folded == false){
						await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to([socket.id]).emit('OnPlayerCards',{
							playerId : room.players[i].id,
			...
			await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to([socket.id]).emit('OnSubscribeRoom', {
				roomId : room.id,
		...
		try {
			...
			for(let i = 0; i< room.players.length; i++){
				if(room.players[i].id == data.playerId && room.players[i].status != 'Left'){
					...
					await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('onIdealPlayer', { 'playerId': room.players[i].id,status : false, roomId:room.id });
				}
			}
	...
	reconnectGame: async function (socket, data){
		try {
			...
			await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(socket.id).emit('ResetGame', {roomId: room.id});
	...
	joinRoom: async function (socket, data){
		try {
			...
			var room = null;
			room = await Sys.Game.Sng.Omaha.Controllers.RoomProcess.joinRoom(player, data);
	...
	leaveRoom: async function (socket, data) {
		try {
			...
			/*let responce = await Sys.Game.Sng.Omaha.Controllers.RoomProcess.leftRoom(data);
	...
	sitOutNextHand: async function (socket, data){
		try {
			let responce = await Sys.Game.Sng.Omaha.Controllers.RoomProcess.sitOutNextHand(data);
	...
	sitOutNextBigBlind: async function (socket, data){
		try {
			let responce = await Sys.Game.Sng.Omaha.Controllers.RoomProcess.sitOutNextBigBlind(data);
			return { status: 'success', result: null, message: responce.message, statusCode: 200 }
		}
		catch (e) {
			console.log('Error in JoinRoom : ', e);
	...
	playerAddChips: async function (socket, data){
		try {
			...
			let playerUpdate = await Sys.Game.Sng.Omaha.Services.PlayerServices.update(player.id, { chips: chips }); // Update Player Chips
	...
	showMyCards: async function (socket, data){
		try {
			if( data.gameId ){
				...
				/*let game = await Sys.Game.Sng.Omaha.Services.GameServices.getSingleGameData({_id: data.gameId});
				...
				await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('ShowCardResult', { playerId : data.playerId, cards:playerCards, roomId: room.id });
				return {
					status: 'success',
					result: {
						gameId : data.gameId ,
