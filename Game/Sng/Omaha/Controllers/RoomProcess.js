...
module.exports = {
	...
	joinRoom: async function (player, data) {
		try {
			let self = this
			var room = await Sys.Game.Sng.Omaha.Services.RoomServices.get(data.roomId);
			...
			if (seatAvailable) {
				...
				let playerUpdate = await Sys.Game.Sng.Omaha.Services.PlayerServices.update(player.id, { chips: chips });
				//Shiv!@#
				...
				if (room.players.length > 0) {
					...
					room  =  await Sys.Game.Sng.Omaha.Controllers.RoomProcess.broadcastPlayerInfo(room);
					...
					if (room.status != 'Running' && totalPlayers >= room.minPlayers) {
						if (room.game == null && room.timerStart == false) {
							...
							room = await Sys.Game.Sng.Omaha.Services.RoomServices.update(room);
							await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('OnGameStartWait',{roomId: room.id})
	...
	/*leftRoom: async function (data) {
		try {
			...
			if (room && room.players && room.players.length > 0) {
				for (let i = 0; i < room.players.length; i++) {
					...
					if (room.players[i].id == data.playerId && room.players[i].status != 'Left') {
						await Sys.Game.Sng.Omaha.Services.playerGameHistoryServices.updatePlayerStatus(room.players[i].id,'Finished');
			...
			if (player) {
				...
				await Sys.Game.Sng.Omaha.Services.RoomServices.update(room);
				console.log("Player Left ", player.id);
				await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('PlayerLeft', { 'playerId': player.id, roomId: room.id });
				let dataPlayer = await Sys.Game.Sng.Omaha.Services.PlayerServices.getById(player.id);
				if (dataPlayer) {
					...
					//var playerUpdate = await Sys.Game.Sng.Omaha.Services.PlayerServices.update(player.id, { chips: chips });
	...
	newRoundStarted: async function (room) {
		try {
			...
			var room = await Sys.Game.Sng.Omaha.Services.RoomServices.update(room)
			if (!room) {
				return {
					status: 'fail',
					result: null,
			...
			await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('OnGameBoot', bootGameData);
			...
			Sys.Timers[room.id] = setTimeout(async function (room) {
				...
				await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('OnPlayersCardsDistribution', { playersCards : playersCards, roomId: room.id })
				let flipAnimation = true;
				Sys.Timers[room.id] = setTimeout(async function (room) {
					// Send Player Cards in his Socket.
					for(let i=0; i < room.players.length; i++){
						if(room.players[i].status == 'Playing'){
							await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to([room.players[i].socketId]).emit('OnPlayerCards',{
								playerId : room.players[i].id,
					...
					if(totalPlayingPlayers == 2 &&  await Sys.Game.Sng.Omaha.Controllers.PlayerProcess.checkForEndOfRound(room) === true  ){	
						Sys.Timers[room.id] = setTimeout(async function (room) {
							console.log("Player Timer Send 4 when two players and forcefinsihallin: ", room.game.gameNumber);
							await Sys.Game.Sng.Omaha.Controllers.PlayerProcess.progress(room);		
							...
							Sys.Timers[room.id] = setTimeout(async function (room) {
								Sys.Timers[room.id] = setInterval(async function (room) {
									...
									if(room.getCurrentPlayer().isIdeal == true){
										...
										Sys.Game.Sng.Omaha.Controllers.RoomProcess.playerDefaultAction(room.id);
									}else{
										...
										if (timer < 0) {
											...
											await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('OnTurnTimer', {
	...
	newGameStarted: async function (room) {
		try {
			...
			await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('GameStarted', {
	...
	turnFinished: async function (room) {
		try {
			...
			if (room.getCurrentPlayer()) {
				...
				// await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('NextTurnPlayer', {
						...
						if(room.getCurrentPlayer().isIdeal == true){
							...
							if (timer < 0) {
								...
								if (maxBet == yourBet) {
									...
									Sys.Game.Sng.Omaha.Controllers.RoomProcess.playerAction({
										playerId: room.getCurrentPlayer().id,
									  	betAmount: 0,
									  	action: 2,
									  	roomId: room.id,
			...
			Sys.Timers[room.id] = setTimeout(async function (room, showCardsPlayerIds) {
				...
				Sys.Timers[room.id] = setTimeout(async function (room, showCardsPlayerIds) {
					...
					if (originalGameStatus != 'Finished AllIn') {
						...
						//await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('DisplayShowCardButton', { playerIdList : showCardsPlayerIds, gameId:room.game.id, roomId: room.id });
					...
					await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('DisplayShowCardButton', { playerIdList: showCardsPlayerIds, gameId: room.game.id, roomId: room.id });
					/*else{
						await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('DisplayShowCardButton', { playerIdList : room.lastFoldedPlayerIdArray, gameId:room.game.id, roomId: room.id });
					}*/
					...
					Sys.Timers[room.id] = setTimeout(async function (room) {
						...
						for (var rp = 0; rp<room.players.length; rp++){
							...
							if(winnerPlayerIds.indexOf(room.players[rp].id) > -1 && parseFloat(totalBetAmount) > 0){
								...
								var transactionDetail = await Sys.Game.Sng.Omaha.Services.ChipsServices.getSingleData({user_id : room.players[rp].id, gameId : room.game.id});
						...
						//await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('GameFinished', { winners: room.gameWinners, roomId: room.id, previousGameNumber : room.previousGameNumber, previousGameId : room.previousGameId });
						await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('GameFinished', { winners: room.gameWinners, roomId: room.id });
						// start new game
						/**
						...
						let history = await Sys.Game.Sng.Omaha.Controllers.RoomProcess.saveGameToHistry(room);
						...
						Sys.Timers[room.id] = setTimeout(async function (room) {
							...
							await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('ResetGame', { roomId: room.id });
							...
							for (let i = 0; i < room.players.length; i++) {
								...
								if( room.players[i].chips == 0){
									...
									room.gameLosers.push(room.players[i]);
									Sys.Game.Sng.Omaha.Services.playerGameHistoryServices.updatePlayerStatus(room.players[i].id,'Finished');
									room.players[i].status = 'Left';
									await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('PlayerLeft', { 'playerId': room.players[i].id, roomId: room.id });
								}
							}
							...
							Sys.Timers[room.id] = setTimeout(async function (room) {
								...
								for (i = 0; i < room.players.length; i++) {
									if (room.players[i].status == 'Ideal' && room.players[i].chips == 0) {
										...
										//await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('OnOpenBuyInPanel', { 'playerId': room.players[i].id, roomId:room.id });
										await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.players[i].socketId).emit('OnOpenBuyInPanel', { 'playerId': room.players[i].id, roomId: room.id });
								...
								if (totalPlayers >= room.minPlayers && room.status != 'Running' && room.timerStart == false) {
									...
									if(minutesDifference >= parseFloat(Sys.Config.Texas.regulaerBreakStartIn)){
										let tournament = await Sys.Game.Sng.Omaha.Services.TournamentServices.getById(room.tournament);
										...
										for(let i= 0;i<tournament.rooms.length;i++){
											...
											let TrRoom = await Sys.Game.Sng.Omaha.Services.RoomServices.get(tournament.rooms[i]);
										...
										Sys.Timers[room.id] = setInterval(async function (room) {
											...
											Sys.Game.Sng.Omaha.Controllers.TournamentProcess.stopBlindTimer(tournament);
											...
											OtherRoom.forEach( async function (allRooms){
												...
												await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(allRooms.id).emit('OnBreak', {
													timer: breakTime,
													name: 'break',
													roomId: allRooms.id
												});
											...
											if (breakTime < 1) {
												...
												setTimeout( async () => {
													Sys.Game.Sng.Omaha.Controllers.TournamentProcess.startBlindTimer(tournament,roomIds);
									...
									Sys.Game.Sng.Omaha.Controllers.PlayerProcess.checkForTournamentWinner(room);
									Sys.Game.Sng.Omaha.Controllers.TournamentProcess.stopBlindTimer(tournament);
								}
	...
	playerDefaultAction: async function (id) {
		try {
			...
			let room = await Sys.Game.Sng.Omaha.Services.RoomServices.get(id);
			if (room.getCurrentPlayer()) {
				...
				for (i = 0; i < room.game.bets.length; i += 1) {
					...
					if (room.game.bets[room.currentPlayer] == maxBet) {
						room.check(currentPlayer.id);
							...
							await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('onIdealPlayer', { 'playerId': currentPlayer.id, status: true, roomId: room.id });
	...
	saveGameToHistry: async function (room) {
		try {
			...
			return await Sys.Game.Sng.Omaha.Services.RoomServices.update(room);
		} catch (e) {
			console.log("Error:", e);
		}
	},
	...
	broadcastPlayerInfo: async function (room) {
		...
		//	console.log("while sending broadcast roomdata", playerInfoDummy);
			...
			await Sys.Io.of(Sys.Config.Namespace.CashSngOmaha).to(room.id).emit('playerInfoList', {
