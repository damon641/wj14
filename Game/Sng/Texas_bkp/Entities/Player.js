...
class Player {
    constructor (id, socketId, seatIndex,playerName, avatar, fb_avatar,status, chips,folded,allIn,talked,cards,autoBuyin,defaultActionCount,isBot,isSidepot,sitOutNextHand,sitOutNextBigBlind) {
    ...
    Check ( roomId,hasRaised ) {
        ...
        if (Sys.Rooms[roomId].game.bets[Sys.Rooms[roomId].currentPlayer] == maxBet) {
           ...
           // console.log(Sys.Rooms[roomId].game.pot);// Shiv!@#
            ...
            Sys.Game.Sng.Texas.Controllers.PlayerProcess.progress(Sys.Rooms[roomId]);
        } else {
    ...
    Call ( roomId, hasRaised) {
        ...
        // let roomData = Sys.Game.Sng.Texas.Services.RoomServices.get(roomId.id)
    ...
    getTotalBet (roomId) {
      // let Sys.Rooms[roomId.id] = Sys.Game.Sng.Texas.Services.RoomServices.get(roomId.id)
        let total = 0;
        let self = this
