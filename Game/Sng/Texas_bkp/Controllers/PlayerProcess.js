...
module.exports = {
        ...
        if (await self.checkForEndOfRound(room) === true || room.game.status == 'ForceFinishedFolded' || room.game.status == 'ForceFinishedAllIn') {
          ...
          if (room.game.status == 'ForceFinishedAllIn') {
            ...
            switch (room.game.roundName) {
              ...
              //Evaluate each hand
              for (j = 0; j < room.players.length; j += 1) {
                  hand = new Sys.Game.Sng.Texas.Entities.Hand(room.players[j].cards, room.game.board
                  )
                  room.players[j].hand = await self.rankHand(hand);
              }
              await self.checkForWinner(room);
  ...
  getHand: async function(player, board) {
    try {
      let hand = await new Sys.Game.Sng.Texas.Entities.Hand(player.cards, board);
  ...
  checkForBankrupt: async function(room) {
    try {
            ...
            if (room.players[i].chips == 0) {
                //if (room.players[i].autoBuyin == 0) {
                    ...
                    if(isRevertAvilabel == false){
                        ...
                        console.log("room.gameLosers.push : ",room.players[i]);
        ...
        //     let players = await Sys.Game.Sng.Texas.Services.PlayerServices.getByIds(ids);
        ...
        //                         await Sys.Game.Sng.Texas.Services.PlayerServices.update({id: player.id}, player);
        ...
        //                         await Sys.Io.of(Sys.Config.Namespace.CashSngTexas).to(room.id).emit('PlayerInfo', tempdata)
        ...
        //                                 // room.players.splice(i, 1);
        //                                 room.players[i].status = 'Left'
        //                                // await load('Games/Sng/Omaha/Controllers/BotController').checkBots(room.id)                                        
        //                             }
        //                         }
        //                     }
        //                 }
    ...
    //  console.log("tempHandCard :",tempHandCard)
      ...
      if (rank === 0) {
        ...
        for (var i = 0; i < cards.length; i++) {
          if (ranks.indexOf(cards[i]) > -1 && rank === 0) {
            let rankKicker = await Sys.Game.Sng.Texas.Controllers.PlayerProcess.rankKickers(ranks.replace(cards[i], ''), 1);
      ...
      if (rank === 0) {
       ...
       // console.log('cards:->>> ', cards);
        if(rank === 128){
          let rankKicker = await Sys.Game.Sng.Texas.Controllers.PlayerProcess.rankKickers(ranks, 5);
      ...
      if (rank === 0) {
        ...
        for (let i = 0; i < cards.length; i++) {
          if (ranks.indexOf(cards[i]) > -1 && rank === 0) {
            let rankKicker = await Sys.Game.Sng.Texas.Controllers.PlayerProcess.rankKickers(ranks.replace(cards[i], ''), 2);
            rank = rnk + rankKicker.rank; setBestCard([cards[i], rankKicker.cards]);
          }
          rnk--;
        }
          ...
          for (let k = 0; k < cards.length; k++) {
            if (cards[i].indexOf(cards[k]) == -1 && k > i) {
              if (ranks.indexOf(cards[i]) > -1 && ranks.indexOf(cards[k]) > -1 && rank === 0) {
                let rankKicker = await Sys.Game.Sng.Texas.Controllers.PlayerProcess.rankKickers(ranks.replace(cards[i], '').replace(cards[k], ''), 1);
      ...
      if (rank === 0) {
        ...
        for (let i = 0; i < cards.length; i++) {
          if (ranks.indexOf(cards[i]) > -1 && rank === 0) {
            let rankKicker = await Sys.Game.Sng.Texas.Controllers.PlayerProcess.rankKickers(ranks.replace(cards[i], ''), 3);
            rank = rnk + rankKicker.rank; setBestCard([cards[i], rankKicker.cards]);
          }
          rnk--;
        }
      ...
      if (rank === 0) {
        ...
        for (let i = 0; i < cards.length; i++) {
          ...
          if (ranks.indexOf(cards[i]) > -1 && rank === 0) {
    //        console.log(">>>>>>>>>>>>>>>>>>>>>> :",cards[i]);
            let rankKicker = await Sys.Game.Sng.Texas.Controllers.PlayerProcess.rankKickers(ranks.replace(cards[i], ''), 4);  
     //       console.log("Ranks >> :",ranks);
  ...
  checkForTournamentWinner: async function (room) {
    try {
      ...
      for (let i = room.gameLosers.length-1; i < room.gameLosers.length-2; i--) {
          tournamentWinners.push({ 
            playerId : room.gameLosers[i].id,
            playerName : room.gameLosers[i].playerName,
      ...
      let prizePoolTotal = parseInt(sngTournament.buy_in * sngTournament.max_players);
      ...
      for (let i = 0; i < tournamentWinners.length; i++) {
        ...
        let winningPrise = 0;
        if(i == 0){
          winningPrise = parseInt(parseInt(total * parseInt(prizePool.winner))/100);
          prizePoolTotal = prizePoolTotal - winningPrise;
        }
        if( i == 1){
          winningPrise = parseInt(parseInt(total * parseInt(prizePool.firstRunnerUp))/100);
          prizePoolTotal = prizePoolTotal - winningPrise;
        }
        if( i >= 3){
          winningPrise = prizePoolTotal;
        }
        console.log("winningPrise ->",winningPrise)


        tournamentWinners[i].chips = winningPrise;

        await Sys.Game.Sng.Texas.Services.PlayerServices.update(tournamentWinners[i].playerId, { chips: parseInt( player.chips + winningPrise )   });
      }
      ...
      //   firstRunnerUp = room.gameLosers.splice(-1);
      //   secondRunnerUp = room.gameLosers.splice(-1);
      // }
      // else{
      //   winner = room.gameLosers.splice(-1);
      //   firstRunnerUp = room.gameLosers.splice(-1);
      //   secondRunnerUp = room.gameLosers.splice(-1);
      // }


      // Touranemrt Winners Log
      // console.log("winner :",winner.id);
      // console.log("firstRunnerUp :",firstRunnerUp.id);
      // console.log("secondRunnerUp :",secondRunnerUp.id);
      ...
      // let winnerPlayerObject = await Sys.Game.Sng.Texas.Services.PlayerServices.getById(winner.id);
      // if(winnerPlayerObject instanceof Error) {
      //   return;
      // }
      // let firstRunnerUpPlayerObject = await Sys.Game.Sng.Texas.Services.PlayerServices.getById(firstRunnerUp.id);
      // if(firstRunnerUpPlayerObject instanceof Error) {
      //   return;
      // }
      // let secondRunnerUpPlayerObject = await Sys.Game.Sng.Texas.Services.PlayerServices.getById(secondRunnerUp.id);
      // if(secondRunnerUpPlayerObject instanceof Error) {
      //   return;
      // }


      // console.log("winnerPlayerObject :",winnerPlayerObject);
      // console.log("firstRunnerUpPlayerObject :",firstRunnerUpPlayerObject);
      // console.log("secondRunnerUpPlayerObject :",secondRunnerUpPlayerObject);


      // let totalPrize = parseInt(sngTournament.buy_in * sngTournament.max_players);
      // let winnerPrize = parseInt(totalPrize * parseInt(prizePool.winner) / 100);
      // let winnerAfterChips = parseInt(winnerPlayerObject.chips + winnerPrize);
      // let firstRunnerUpPrize = parseInt(totalPrize * parseInt(prizePool.firstRunnerUp) / 100);
      // let firstRunnerUpAfterChips = parseInt(firstRunnerUpPlayerObject.chips + firstRunnerUpPrize);
      // let secondRunnerUpPrize = parseInt(totalPrize * parseInt(prizePool.secondRunnerUp) / 100);
      // let secondRunnerUpAfterChips = parseInt(secondRunnerUpPlayerObject.chips + secondRunnerUpPrize);

      // room.tournamentWinners.push({
      //   playerId: winner.id,
      //   playerName: winner.playerName,
      //   amount: winnerPrize,
      //   winningType: 'winner',
      //   chips: parseInt(winner.chips + winnerPrize),
      //   prizePool: parseInt(prizePool.winner),
      //   winnerSeatIndex : winner.seatIndex,
      //   sidePotPlayerIndex: -1, // main Port index
      // });
      // room.tournamentWinners.push({
      //   playerId: firstRunnerUp.id,
      //   playerName: firstRunnerUp.playerName,
      //   amount: firstRunnerUpPrize,
      //   winningType: 'firstRunnerUp',
      //   chips: parseInt(firstRunnerUp.chips + firstRunnerUpPrize),
      //   prizePool: parseInt(prizePool.firstRunnerUp),
      //   winnerSeatIndex : firstRunnerUp.seatIndex,
      //   sidePotPlayerIndex: -1, // main Port index
      // });
      // room.tournamentWinners.push({
      //   playerId: secondRunnerUp.id,
      //   playerName: secondRunnerUp.playerName,
      //   amount: secondRunnerUpPrize,
      //   winningType: 'secondRunnerUp',
      //   chips: parseInt(secondRunnerUp.chips + secondRunnerUpPrize),
      //   prizePool: parseInt(prizePool.secondRunnerUp),
      //   winnerSeatIndex : secondRunnerUp.seatIndex,
      //   sidePotPlayerIndex: -1, // main Port index
      // });

      // await Sys.Game.Sng.Texas.Services.PlayerServices.update(winner.id, {
      //   chips: winnerAfterChips
      // });
      // await Sys.Game.Sng.Texas.Services.PlayerServices.update(firstRunnerUp.id, {
      //   chips: firstRunnerUpAfterChips
      // });
      // await Sys.Game.Sng.Texas.Services.PlayerServices.update(secondRunnerUp.id, {
      //   chips: secondRunnerUpAfterChips
      // });

      // // Add chipsHistory
      // await Sys.Game.Common.Services.ChipsHistoryServices.insertChipsData({
      //   user_id: winner.id,
      //   username: winner.playerName,
      //   chips: winnerPrize,
      //   type: 'Debit',
      //   reason: 'SNG Tournament Winner Amount',
      //   gameId: sngTournament.id,
      //   transactionNumber: '',
      //   previousBalance: winnerPlayerObject.chips,
      //   afterBalance: winnerAfterChips,
      //   isValid: true,
      // });
      // await Sys.Game.Common.Services.ChipsHistoryServices.insertChipsData({
      //   user_id: firstRunnerUp.id,
      //   username: firstRunnerUp.playerName,
      //   chips: firstRunnerUpPrize,
      //   type: 'Debit',
      //   reason: 'SNG Tournament First Runner Up Amount',
      //   gameId: sngTournament.id,
      //   transactionNumber: '',
      //   previousBalance: firstRunnerUpPlayerObject.chips,
      //   afterBalance: firstRunnerUpAfterChips,
      //   isValid: true,
      // });
      // await Sys.Game.Common.Services.ChipsHistoryServices.insertChipsData({
      //   user_id: secondRunnerUp.id,
      //   username: secondRunnerUp.playerName,
      //   chips: secondRunnerUpPrize,
      //   type: 'Debit',
      //   reason: 'SNG Tournament Second Runner Up Amount',
      //   gameId: sngTournament.id,
      //   transactionNumber: '',
      //   previousBalance: secondRunnerUpPlayerObject.chips,
      //   afterBalance: secondRunnerUpAfterChips,
      //   isValid: true,
      // });
      ...
      await Sys.Io.of(Sys.Config.Namespace.CashSngTexas).to(room.id).emit('SngTournamentFinished', { winners: tournamentWinners });
    }
    catch (e) {
      console.log("Error in checkForWinner :", e);
      return new Error(e);
