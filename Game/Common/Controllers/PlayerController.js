...
module.exports = {
            ...
            if (data.os && data.appVersion) {
                if ((data.os == 'android' && data.appVersion >= Sys.Setting.android_version) || (data.os == 'ios' && data.appVersion >= Sys.Setting.ios_version) || (data.os == 'other')) {
                    ...
                    if (passwordTrue) {
                        ...
                        if (player.lastChipsAddedDate) {
                            if (player.lastChipsAddedDate < today_Date) {
                                dailyChips = settings.dailyChips;
                            }
                        } else {
                            dailyChips = settings.dailyChips;
                        }
                        ...
                        await Sys.Game.Common.Services.PlayerServices.updatePlayerData({
                            ...
                            socketId: socket.id,
                            platform_os: data.os,
                            chips: player.chips + dailyChips,
                            lastChipsAddedDate: today_Date,
                        });
    ...
    reconnectPlayer: async function(socket, data) {
        try {
            ...
            if (data.playerId) {
                ...
                await Sys.Game.Common.Services.PlayerServices.updatePlayerData({ _id: data.playerId }, {
                    socketId: socket.id,
                });
    ...
    updateProfile: async function(socket, data) {
        try {
            ...
            if (player) {
                ...
                updatedPlayer = await Sys.Game.Common.Services.PlayerServices.update({ _id: player.id }, { firstname: data.firstname, lastname: data.lastname, mobile: data.mobile, gender: data.gender });
                if (updatedPlayer) {
    ...
    claimChips: async function(socket, data) {
        try {
            let player = await Sys.Game.Common.Services.PlayerServices.getOneByData({ _id: data.playerId });
            console.log("player : ", player);
            if (player) {
                let updatedPlayer = false;
                updatedPlayer = await Sys.Game.Common.Services.PlayerServices.updatePlayerData({
                    _id: data.playerId
                }, {
                    chips: player.chips + data.chips
                });
                ...
                return {
                    status: 'success',
                    result: playerData,
                    message: 'Player Chips Added Successfully!'
                }
            } else {
                return {
                    status: 'fail',
    ...
    getVideoLink: async function(socket, data) {
        try {
            let video_primary = await Sys.Game.Common.Services.PlayerServices.getVideoOneByData({ status: 'primary' });
            let video_secondary = await Sys.Game.Common.Services.PlayerServices.getVideoOneByData({ status: 'secondary' });
            let firstadname = '';
            let firstadvideo = '';
            let firstadchips = '';
            let secondadname = '';
            ...
            let secondadchips = '';
            if (video_primary) {
                firstadname = video_primary.name;
                firstadvideo = "http://18.225.32.71/video/" + video_primary.video_name;
                firstadchips = video_primary.chips;
            }
            if (video_secondary) {
                secondadname = video_secondary.name;
                secondadvideo = "http://18.225.32.71/video/" + video_secondary.video_name;
                secondadchips = video_secondary.chips;
            }

            console.log(firstadvideo);
            console.log(secondadvideo);
            return {
                status: 'success',
                message: 'Ad Data Found',
                result: {
                    FirstAdName: firstadname,
                    FirstAdVideoUrl: firstadvideo,
                    FirstAdChips: firstadchips,
                    SecondAdName: secondadname,
                    SecondAdVideoUrl: secondadvideo,
                    SecondAdChips: secondadchips,
                }
            }
        ...
        try {
            ...
            if (player) {
                ...
                var mailOptions = {
                    ...
                    from: 'White Poker',
                    subject: 'White Poker New Password',
            ...
            if (player) {
                ...
                if (bcrypt.compareSync(data.oldPassword, player.password)) {
                    ...
                    return {
                        status: 'fail',
                        result: null,
                        message: 'Sorry ! Your old password was incorrect. Please try again.',
                        statusCode: 400
                    }
                }
            }
            ...
            if ((current_date >= m_start_date && current_date <= m_end_date && Sys.Setting.maintenance.status == 'active') || Sys.Setting.maintenance.quickMaintenance == "active") {
                ...
                if (receiverPlayer && player) {
                    if (await bcrypt.compare(data.password, player.password)) {
                        ...
                        return { status: 'fail', result: null, message: 'Sorry! Your password does not match.', statusCode: 400 }
                    }
                } else {
