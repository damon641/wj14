...
const Model  = mongoose.model('inAppPurchase');
const ServiceModel = mongoose.model('setting')
...
module.exports = {
	...
	updateSettingsData: async function(condition, data){
		try {
			return await ServiceModel.update(condition, data);
		} catch (e) {
			console.log("Error",e);
          	throw new Error('error in updateSettingsData of settings model'+ e.message);
		}
	},

	getSingleSettingsData: async function(data, select, setOption){
        try {
          return  await ServiceModel.findOne(data, select, setOption);
        } catch (e) {
          console.log("Error",e);
          throw new Error('error in getSingleSettingsData of '+ e.message);
        }
	},
