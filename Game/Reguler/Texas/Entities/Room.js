...
class Room {
  constructor(id, tableType, currencyType, name, smallBlind, bigBlind,dealerIndex,smallBlindIndex,bigBlindIndex, minPlayers, maxPlayers, minBuyIn, maxBuyIn, type, gameLimit, status, dealer, players, gameWinners, gameLosers, turnBet, game, currentPlayer, rackPercent, rackAmount, expireTime, owner, tableNumber,timerStart,limit,gameType,isCashGame,otherData,isTournamentTable,tournamentType,tournament, lastFoldedPlayerIdArray,currentBlindIndex,breakTimeCheck,roomStatusCheckOnBreak,updateAt,betCounter,allInInLimit) {
    ...
    this.players = [];
      if (players && Array.isArray(players)) {
        ...
        players.forEach(function (player) {
          ...
          room.players.push(new Sys.Game.Reguler.Texas.Entities.Player().createObject(player));
    ...
    if (gameLosers) {
      gameLosers.forEach(function (player) {
       room.gameLosers.push(new Sys.Game.Reguler.Texas.Entities.Player().createObject(player));
    ...
    if (game) {
      ...
      this.game = new Sys.Game.Reguler.Texas.Entities.Game().createObject(game);
    ...
    this.currentBlindIndex = currentBlindIndex;
    this.breakTimeCheck = (breakTimeCheck) ? breakTimeCheck : false;
    this.updateAt = updateAt;
    this.roomStatusCheckOnBreak = (roomStatusCheckOnBreak) ? roomStatusCheckOnBreak : false;
    this.betCounter = 0;
  ...
  toJson() {
    ...
    var room = {
      ...
      breakTimeCheck : this.breakTimeCheck,
      roomStatusCheckOnBreak : this.roomStatusCheckOnBreak,
  ...
  AddPlayer(id, socketId, playerName, avatar, fb_avatar, chips, seatIndex, autoBuyin, isIdeal) {
    ...
    if (chips >= this.minBuyIn) {
      let player = new Sys.Game.Reguler.Texas.Entities.Player(id, socketId,seatIndex, playerName, avatar, fb_avatar, "Waiting", chips,0,chips,false,false,false,[],autoBuyin,0,false,false,false,false,false,null, false, 0, false, false, false, false, isIdeal, 0, false);
     ...
     Sys.Game.Reguler.Texas.Services.playerGameHistoryServices.create(playerGameObj);
  ...
  async StartGame() {
    ...
    //    //await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('PlayerLeft', { 'playerId':  room.players[room.bigBlindIndex].id });
    //   await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('onIdealPlayer', { 'playerId': room.players[room.bigBlindIndex].id,status : true });
    ...
    if (room.game == null && playersLength >= this.minPlayers) {
      // Tournament Update new Small Blind & BigBlind For Room.
      ...
      let game = await Sys.Game.Reguler.Texas.Services.GameServices.create(gameobj);
      ...
      console.log("----------------------------------------------------------------------");
        ...
        console.log("regular tournament small blind totalGamePot: ", totalGamePot);
        ...
        console.log("regular tournament big blind gamePortData: ", gamePortData);
        ...
        if (playersLength >= room.minPlayers) {
           room.game.status = 'Running';
           room.status = 'Running';
           let gameStarted = await Sys.Game.Reguler.Texas.Controllers.RoomProcess.newGameStarted(room);
           room.initNewRound();
        }
  ...
  async initNewRound() {
    ...
    for (let i = 0; i < room.players.length; i += 1) {
      ...
      player.statistics.tournament.noOfPlayedGames++; 
      ...
      await Sys.Game.Reguler.Texas.Services.PlayerServices.update(player.id, { statistics: player.statistics });
    }

    console.log("Befor Fill Deck")
    room.game.deck = [];
    await new Sys.Game.Reguler.Texas.Entities.Deck().fillDeck(room.game.deck);
    room.NewRound();
  };
  ...
  async NewRound() {
    ...
    // get currentPlayer
     ...
     let newRoundStarted = Sys.Game.Reguler.Texas.Controllers.RoomProcess.newRoundStarted(room);
