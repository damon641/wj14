...
module.exports = {
		...
		try {
			...
			if (seatAvailable) {
				...
				if(room.game){
					...
					await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to([player.socketId]).emit('OnSubscibePlayersCards', { playersCards : playersCards, roomId:room.id });
					...
					for(let i=0; i < room.players.length; i++){
						...
						if(room.players[i].id == player.id && room.players[i].status == 'Playing' && room.players[i].cards.length == 2 && room.players[i].folded == false){
							await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to([room.players[i].socketId]).emit('OnPlayerCards',{
								playerId : room.players[i].id,
	...
	/*joinRoom: async function (player, data) {
		try {
			...
			if (seatAvailable) {
				...
				room = await Sys.Game.Reguler.Texas.Services.RoomServices.update(room);
				console.log("Room Updated");
				let playerUpdate = await Sys.Game.Reguler.Texas.Services.PlayerServices.update(player.id, { chips: chips });
				//Shiv!@#
				...
				if (room.players.length > 0) {
					...
					if (room.status != 'Running' && totalPlayers >= room.minPlayers) {
						if (room.game == null && room.timerStart == false) {
							...
							room = await Sys.Game.Reguler.Texas.Services.RoomServices.update(room);
							await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('OnGameStartWait',{})
	...
	leftRoom: async function (data) {
		try {
			...
			if (player) {
				...
				await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('PlayerLeft', { 'playerId': player.id, roomId:room.id });
				...
				if (dataPlayer) {
					...
					/*await Sys.Game.Reguler.Texas.Services.PlayerServices.update(player.id, { chips: chips,rooms : dataPlayer.rooms });
					await Sys.Game.Reguler.Texas.Services.playerGameHistoryServices.updatePlayerStatus(player.id,'Finished');
					room  =  await Sys.Game.Reguler.Texas.Controllers.RoomProcess.broadcastPlayerInfo(room);*/
	...
	newRoundStarted: async function (room) {
		try {
			...
			await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('OnGameBoot', bootGameData);
			...
			Sys.Timers[room.id] = setTimeout(async function (room) {
				...
				await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('OnPlayersCardsDistribution', { playersCards : playersCards, roomId: room.id })
				...
				Sys.Timers[room.id] = setTimeout(async function (room) {
					...
					for(let i=0; i < room.players.length; i++){
						if(room.players[i].status == 'Playing'){
							...
							await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('OnPlayerCards',{
								playerId : room.players[i].id,
								cards : room.players[i].cards,
								roomId: room.id,
					...
					//if(totalPlayingPlayers == 2 && ( room.getCurrentPlayer().chips <= 0 || await Sys.Game.Reguler.Texas.Controllers.PlayerProcess.checkForEndOfRound(room) === true ) ){
					
					if(totalPlayingPlayers == 2 &&  await Sys.Game.Reguler.Texas.Controllers.PlayerProcess.checkForEndOfRound(room) === true  ){	
						Sys.Timers[room.id] = setTimeout(async function (room) {
							console.log("Player Timer Send 4 when two players and forcefinsihallin: ", room.game.gameNumber);
							await Sys.Game.Reguler.Texas.Controllers.PlayerProcess.progress(room);		
						...
						if(room.getCurrentPlayer().isIdeal == true){
							...
							if (timer < 0) {
								...
								if (maxBet == yourBet) {
									...
									Sys.Game.Reguler.Texas.Controllers.RoomProcess.playerAction({
										playerId: room.getCurrentPlayer().id,
									  	betAmount: 0,
									  	action: 2,
									  	roomId: room.id,
		...
		try {
			...
			Sys.Timers[room.id] = setTimeout(async function (room, showCardsPlayerIds) {
				...
				Sys.Timers[room.id] = setTimeout(async function (room, showCardsPlayerIds) {
					...
					if (originalGameStatus != 'Finished AllIn') {	
						await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('GameFinishedPlayersCards', { playersCards: playersCards, roomId: room.id })	
					}
					...
					await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('DisplayShowCardButton', { playerIdList: showCardsPlayerIds, gameId: room.game.id, roomId: room.id });
					/*else{
						await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('DisplayShowCardButton', { playerIdList : room.lastFoldedPlayerIdArray, gameId:room.game.id, roomId: room.id });
					}*/
					...
					Sys.Timers[room.id] = setTimeout(async function (room) {
						...
						for (var rp = 0; rp<room.players.length; rp++){
							...
							if(winnerPlayerIds.indexOf(room.players[rp].id) > -1 && parseFloat(totalBetAmount) > 0){
								...
								var transactionDetail = await Sys.Game.Reguler.Texas.Services.ChipsServices.getSingleData({user_id : room.players[rp].id, gameId : room.game.id});
								...
								if(transactionDetail == null){
									...
									await Sys.Game.Reguler.Texas.Services.ChipsServices.insertData(transactionDataWinData);
						...
						//room = await Sys.Game.Reguler.Texas.Controllers.RoomProcess.rackDeduction(room);
						...
						await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('GameFinished', { winners: room.gameWinners, roomId: room.id });
						...
						Sys.Timers[room.id] = setTimeout(async function (room) {
							...
							await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('ResetGame', {roomId: room.id});
							...
							for (let i = 0; i < room.players.length; i++) {
								/*if(room.players[i].sitOutNextHand == true){
									...
									//await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('PlayerLeft', { 'playerId': room.players[i].id, roomId: room.id });
									await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('onIdealPlayer', { 'playerId': room.players[i].id,status : true });
								...
								if( room.players[i].chips == 0){
							        ...
							        console.log("regular tournament player chips zero transactionData: ", transactionData);
							        await Sys.Game.CashGame.Texas.Services.PlayerAllTransectionService.createTransaction(transactionData);


									await Sys.Game.Reguler.Texas.Services.playerGameHistoryServices.updatePlayerStatus(room.players[i].id,'Finished');
									await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('PlayerLeft', { 'playerId': room.players[i].id, roomId: room.id });
									 tournament = await Sys.Game.Reguler.Texas.Services.TournamentServices.getById(room.tournament);
						...
						/*	room.gameLosers.forEach(async function (player) {
								...
								console.log('<=> Game PlayerLeft Broadcast ||  Texas GAME-NUMBER [] || PlayerLeft : ', player.id);
								await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('PlayerLeft', { 'playerId': player.id, roomId:room.id });
							})*/
							...
							/*for (i = 0; i < room.players.length; i++) {
								if(room.players[i].status == 'Ideal' && room.players[i].chips == 0 && parseFloat(room.players[i].extraChips) == 0 ){ 
									...
									if(room.players[i].entryChips <  maxBuyIn){
										...
										//await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('OnOpenBuyInPanel', { 'playerId': room.players[i].id, roomId:room.id });
										await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.players[i].socketId).emit('OnOpenBuyInPanel', { 'playerId': room.players[i].id, roomId:room.id });
									}else{
										return {
											status: 'fail',
											result: null,
							...
							// 		let TourRoom = await Sys.Game.Reguler.Texas.Services.RoomServices.get(tournamentRoomsIdsArray[i].roomId);
							// 		console.log('Room :',TourRoom.id);
							...
							if(mergeRoom){
								...
								let tournamentRooms = [];
								for(let j = 0; j < tournament.rooms.length; j++){	
									let roomsOfTournament = await Sys.Game.Reguler.Texas.Services.RoomServices.get(tournament.rooms[j]);
									tournamentRooms.push(roomsOfTournament)
								}
								...
								for (i = 0; i < room.players.length; i++) {
									if(room.players[i].status != 'Left'){
										...
										console.log("merged player", room.players[i].playerName, room.players[i].chips,tournament.rooms)
										console.log("ALL ROOMS STATUS",tournamentRooms.every((val)=> val.status))
										if(tournamentRooms.every((val)=> val.status != 'Running')){
											await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(mergeRoom).emit('OnTournamentTableBreakMessage',{
												roomId : mergeRoom,
												message: ''
											})
											let finalRoom =  await Sys.Game.Reguler.Texas.Services.RoomServices.get(mergeRoom);
											finalRoom.roomStatusCheckOnBreak = false;
										}
											
										let joinTourna= await Sys.Game.Reguler.Texas.Controllers.RoomProcess.joinTournamentRoom(playerObj,mergeRoom);
									}
								}
								...
								await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('OnSwitchRoom', {
									...
									let mergerRoom = await Sys.Game.Reguler.Texas.Services.RoomServices.get(mergeRoom);
								...
								if(totalPlayers >= room.minPlayers){
									...
									if(minutesDifference >= parseFloat(Sys.Config.Texas.regulaerBreakStartIn)){
										...
										for(let i= 0;i<tournament.rooms.length;i++){
											...
											TrRoom = await Sys.Game.Reguler.Texas.Services.RoomServices.get(tournament.rooms[i]);
										...
										if(OtherRoom.every((val, i, arr) => val.status === arr[0].status)){
											console.log("COMPARE TIME AND ROOM ID",compareTime.id,room.id)
											if(compareTime.id == room.id){
												Sys.Timers[room.id] = setInterval(async function (room) {
													console.log("Break Start :",breakTime);
													//Sys.Game.Reguler.Texas.Controllers.TournamentProcess.stopBlindTimer(tournament,breakTime);
													Sys.Game.Reguler.Texas.Controllers.TournamentProcess.stopBlindTimer(tournament);
													...
													OtherRoom.forEach( async function (allRooms){
														...
														await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(allRooms.id).emit('OnBreak', {
															timer: breakTime,
															name: 'break',
															roomId: allRooms.id
														});
													...
													if (breakTime < 1) {
														...
														//Sys.Game.Reguler.Texas.Controllers.TournamentProcess.startBlindTimer(tournament,roomIds,room.otherData.timerCheck,room.otherData.blindTimerCheck);
														Sys.Game.Reguler.Texas.Controllers.TournamentProcess.startBlindTimer(tournament,roomIds);
											...
											if(room.status == 'Finished'){
												await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('OnTournamentTableBreakMessage',{
													roomId : room.id,
	...
	playerDefaultAction: async function (id) {
		try {
			...
			if (room.getCurrentPlayer()) {
				...
				//await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('onIdealPlayer', { 'playerId': currentPlayer.id,status : true });
				...
				if (room.game.bets[room.currentPlayer] == maxBet) {
					room.check(currentPlayer.id);
						...
						// When Player is ideal, Fold theme
						await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('onIdealPlayer', { 'playerId': currentPlayer.id, status: true, roomId: room.id });
						//currentPlayer.status = 'Ideal';
	  ...
	  try{
	  	...
	  	for (let i = 0; i < room.gameWinners.length; i++) {
	        ...
	        if(gamePortData != null){
		        ...
		        console.log("regular tournament rake deduct transactionDataPot: ", transactionDataPot)
