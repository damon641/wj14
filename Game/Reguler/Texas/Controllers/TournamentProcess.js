...
module.exports = {
	...
	startTournament: async function (data) {
      ...
      let blinds = await Sys.Game.Common.Services.blindLevelsServices.getBlindLevelsData(tournament.blindLevels);
            ...
            // Create Empty Rooms
            let roomCounter = await Sys.Game.Reguler.Texas.Services.RoomServices.getLastTable();
            let roomObj = {
    ...
    finishTournament: async function (roomId,tournamentId) {
		try {
            ...
            let payOutRecords = await Sys.Game.Reguler.Texas.Services.pricePoolServices.getPricePoolDataSelect( {minPlayers: { $lte:  tournament.players.length}} );
            console.log("payOutRecords", payOutRecords, tournament.isFreeRoll);    
                if(payOutRecords[0]){
                    let prisePoolArray = [];
                    for (let [key, value] of Object.entries(payOutRecords[0].toObject())) {console.log("key value", key, value);
                    ...
                    console.log("pricepool array", prisePoolArray);
                    let playout = [];
                    ...
                    console.log("payout for tournament", playout, tournament.isFreeRoll);

                    if(tournament.isFreeRoll == true){
                        let first = true;
                        for(let i=0;i<playout.length;i++){
                          let player = await Sys.Game.Reguler.Texas.Services.PlayerServices.getById(tournament.tournamentLosers[ tournament.tournamentLosers.length-1]);    
                            ...
                            //deduct freeroll amount from admin account end
                            registeredPlayersArray.splice( registeredPlayersArray.indexOf(player.id), 1 );
                            await Sys.Game.Reguler.Texas.Services.PlayerServices.update(player.id,{ 
                    ...
                    console.log("/****************************************************************/")
                    await Sys.Game.Reguler.Texas.Services.playerGameHistoryServices.updatePlayerStatus(tournament.tournamentWinners[0].playerId,'Finished');
                    await Sys.Game.Reguler.Texas.Services.TournamentServices.updateTourData({_id: tournamentId},{status:'Finished',tournamentWinners : tournament.tournamentWinners,tournamentLosers:registeredPlayersArray,tournamentTotalChips:parseFloat(tournament.tournamentTotalChips).toFixed(4)});
...
/*    stopBlindTimer: async function (tournament){
    ...
    startBlindTimer: async function (tournament,newRoomIds,timerCheck = 0,blindTimerCheck = 0 ){
      console.log("TIMER CHECK",timerCheck)
      console.log("BLIND TIMER CHECK",blindTimerCheck)
      ...
      Sys.Timers[tournament.id] = setInterval(async function(){
          ...
          if(timerCheck != 0){
            console.log("YES")
            difference = timerCheck - blindTimerCheck;
          }else{
            console.log("NO")
            difference = current - blindTimer;
          }
          console.log("BLIND TIMER",blindTimer)
          console.log("CURRENT",current)
          console.log("DIFF",difference)
          let minutesDifference = Math.floor(difference/1000/60);
          let secondsDifference = Math.floor(difference/1000);
          console.log("SEC DIFF",secondsDifference)
            for(let i=0;i<newRoomIds.length;i++){
              let room = await Sys.Game.Reguler.Texas.Services.RoomServices.get(newRoomIds[i]);
              room.otherData.timerCheck = current;
              if(blindTimerCheck == 0){
                room.otherData.blindTimerCheck = blindTimer;
              }
              console.log("ROOM OTHER DATA",room.otherData)
              ...
              await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(newRoomIds[i]).emit('OnBlindLevels', {
                  roomId: newRoomIds[i],
                  current : {
                  ...
                  remain : parseFloat(parseFloat(Sys.Tournaments[tournament.id].blind_levels_rise_time * 60) - parseFloat(secondsDifference))
              });
           } 
          ...
          if(minutesDifference >= parseFloat(Sys.Tournaments[tournament.id].blind_levels_rise_time)){
              console.log("--------------------------------------------")
              ...
              let room;
              for(let i=0;i<newRoomIds.length;i++){
                room = await Sys.Game.Reguler.Texas.Services.RoomServices.get(newRoomIds[i]);
                room.currentBlindIndex++;
                 // let room = await Sys.Game.Reguler.Texas.Services.RoomServices.get(newRoomIds[i]);
                  ...
                  await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(newRoomIds[i]).emit('OnBlindLevelsRaised', {
                      tournamentId: tournament.id,
                      type : 'reguler',
      ...
      try {
        ...
        for (let index = 0; index < agentLevel.level; index++) {
          ...
          if (agent) {
            ...
            await   ({ _id: mongoose.Types.ObjectId(agentId) }, {
