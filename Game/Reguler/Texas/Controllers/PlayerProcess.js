        ...
        /*for (let j = 0; j < room.players.length; j += 1) {
            if ( room.players[j].status === 'Ideal' ) {
                  ...
                  if(resultInMinutes >= 2){
                    ...
                    await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('PlayerLeft', { 'playerId': room.players[j].id, roomId:room.id });
                    let playerInfoDummy = [];
                    ...
                    await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to([room.players[j].socketId]).emit('playerInfoList', {
          ...
          if (room.game.status == 'ForceFinishedAllIn') {
            ...
            /*if(room.game.status == 'ForceFinishedFolded'){
              ...
              if(room.otherData.lastFoldedPlayerId){
                 Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('DisplayShowCardButton', { playerIdList : [room.otherData.lastFoldedPlayerId], gameId: room.game.id, roomId: room.id });
              }
            }*/
       ...
       let playerIndexes = room.game.gamePot[h].playerIndex;
        ...
        for (let s = 0; s < PokerWinnerResult.length; s++) {
           ...
           if(PokerWinnerResult.length == PokerWinnnerHands.length){
            ...
            let player = await Sys.Game.Reguler.Texas.Services.PlayerServices.getById(winplr.id);
      ...
      if (room.game.gameRevertPoint[h].amount > 0) {
        ...
        if(isFolded == true){
          ...
          if(dataObj.forcefinishfolded == true){
            ...
            await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(room.id).emit('RevertPointFolded', dataObj);
