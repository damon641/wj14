...
module.exports = {
  ...
  get: async function(id){
    try {
      if(Sys.Rooms[id]){
        ...
        Sys.Rooms[room.id]  = new Sys.Game.Reguler.Texas.Entities.Room().createObject(room); // Save room in Game object
  ...
  create: async function(data){
    ...
    try {
      let roomSave = new roomModel(data);
      let room = await roomSave.save(); // Save Room
      if(room){
        Sys.Rooms[room.id] = new Sys.Game.Reguler.Texas.Entities.Room().createObject(room);
  ...
  getLastTable: async function () {
    try {
      return await roomModel.find({}).limit(1).sort({$natural:-1});
    } catch (err) {
