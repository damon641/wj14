...
module.exports = {
  ...
  create: async function(data){Sys.Log.info('<=> gameCount');
    ...
    try {
      ...
      let gameCount = await gameModel.findOne({}, {gameNumber: 1} ).sort({$natural:-1}).limit(1);
      Sys.Log.info('<=> gameCount|| '+gameCount);
      let gameNumber = 'G' + ( parseInt(gameCount.gameNumber.slice(1) ) + 1); // Cash Poker Texas [G]
     ...
     let game = await gameSave.save(); // Save Game
      if(game){
        return new Sys.Game.Reguler.Texas.Entities.Game().createObject(game) // return Game
      }else{
