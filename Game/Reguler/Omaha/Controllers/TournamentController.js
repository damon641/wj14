...
module.exports = {
	...
	startTournament: async function (data){
		try {
            ...
            Sys.Log.info('*****************************************************');
            await Sys.Game.Reguler.Omaha.Controllers.TournamentProcess.startTournament(data);
            console.log("After....");
		}
		catch (error) {
	...
	joinTournament: async function (socket, data){
		try {
			...
			let room = await Sys.Game.Reguler.Omaha.Controllers.TournamentProcess.joinTournament(data);
