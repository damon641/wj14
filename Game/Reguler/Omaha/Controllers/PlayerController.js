...
module.exports = {
    ...
    tip: async function(socket,data){
        try {
          ...
          let player = await Sys.Game.Reguler.Omaha.Services.PlayerServices.getByData({ _id :socket.playerId })
          if (player) {
             var chips = player.chips - parseFloat(data.chips);
             let update = await Sys.Game.Reguler.Omaha.Services.PlayerServices.update(socket.playerId, {chips: chips} );
            ...
            await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to(data.roomId).emit('Tip', data)
             return {
               status: 'success',
               result:  null,
    ...
    chat: async function(socket, data){
      try {
        await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to(data.roomId).emit('Chat', data)
          return {
            status  : 'success',
            result  : null,
    ...
    gift: async function(socket, data){
      try {
        await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to(data.roomId).emit('PlayerGift', data)
        return {
          status: 'success',
          result  : null,
    ...
    inviteFriend: async function(socket, data){
      try {
        let player = await Sys.Game.Reguler.Omaha.Services.PlayerServices.getById(data.friendId);
        if (!player) {
            return {
                status: 'fail',
                result: null,
        ...
        let sender = await Sys.Game.Reguler.Omaha.Services.PlayerServices.getById(data.playerId);
        if (!sender) {
            return {
                status: 'fail',
                result: null,
        ...
        let room = await Sys.Game.Reguler.Omaha.Services.RoomServices.get(gamePlayer.room);
        if (!room) {
            return {
                status: 'fail',
                result: null,
        ...
        await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to(data.roomId).emit('InviteFriend', gamePlayer)
        return {
            status : 'success',
            result : 'ok',
    ...
    playerAction: async function(socket, data){
      try {
        var action = await Sys.Game.Reguler.Omaha.Controllers.RoomProcess.playerAction(data);
        return {
          status: 'success',
          result  : null,
          data: action,
    ...
    playerDetails: async function(socket, data){
      try {
        let player = await Sys.Game.Reguler.Omaha.Services.PlayerServices.getById(data.id);
        if (player) {
          return {
              status: 'success',
              result: data,
    ...
    removePlayerFromRooms:async function() {
      console.log("remove player calles")
        try{
          ...
          let allRooms = await Sys.Game.Reguler.Omaha.Services.RoomServices.getAllRoom({});
                  ...
                  for(let p =0; p < players.length; p++){
                    ...
                    if(playerTimeout != null){
                      ...
                      if( removeAt < moment() ){
                        ...
                        await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to(room.id).emit('PlayerLeft', { 'playerId': room.players[p].id, roomId: room.id });
                        console.log("ideal player status", room.players[p].isAllinPlayersChipsAssigned)
                        let dataPlayer = await Sys.Game.Reguler.Omaha.Services.PlayerServices.getById(room.players[p].id);
                        if (dataPlayer && room.players[p].isAllinPlayersChipsAssigned == false) {
                          ...
                          var playerUpdate = await Sys.Game.Reguler.Omaha.Services.PlayerServices.update(room.players[p].id, { chips: chips });
                          ...
                          console.log("omaha tournament left player transactionLeftData: ", transactionLeftData);
                          await Sys.Game.CashGame.Texas.Services.PlayerAllTransectionService.createTransaction(transactionLeftData);    
                          
                        }
                        let playerUpdated = await Sys.Game.Reguler.Omaha.Services.RoomServices.update(room)
                ...
                if(room.status != 'Running' && totalPlayingPlayers == 1){
                  ...
                  if(subscribePlayerTIme != null){
                    ...
                    if( removeAt < moment() ){
                      ...
                      await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to(room.id).emit('PlayerLeft', { 'playerId': room.players[removePlayerIndex].id, roomId:room.id });
                      let dataPlayer = await Sys.Game.Reguler.Omaha.Services.PlayerServices.getById(room.players[removePlayerIndex].id);
                      console.log("single player status", room.players[removePlayerIndex].isAllinPlayersChipsAssigned)
                        if (dataPlayer && room.players[removePlayerIndex].isAllinPlayersChipsAssigned == false) {
                          ...
                          var playerUpdate = await Sys.Game.Reguler.Omaha.Services.PlayerServices.update(room.players[removePlayerIndex].id, { chips: chips });
                          ...
                          console.log("omaha tournament Player left for game: ", transactionDataLeft);
          ...
          console.log("Error when ideal player remove error: ", error);
        }
    },
}
