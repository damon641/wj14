...
module.exports = {
	...
	subscribeRoom: async function (socket, data){
		try {
			...
			var player = await Sys.Game.Reguler.Omaha.Services.PlayerServices.getById(data.playerId);
			let room = await Sys.Game.Reguler.Omaha.Controllers.RoomProcess.checkRoomSeatAvilability(data);
			...
			if(room.game){
				...
				await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to([socket.id]).emit('OnSubscibePlayersCards', { playersCards : playersCards,roomId:room.id });
				...
				for(let i=0; i < room.players.length; i++){
					...
					if(room.players[i].id == data.playerId && room.players[i].status == 'Playing' && room.players[i].cards.length == 4 && room.players[i].folded == false){
						await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to([socket.id]).emit('OnPlayerCards',{
							playerId : room.players[i].id,
			...
			await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to([socket.id]).emit('OnSubscribeRoom', {
				roomId : room.id,
	...
	playerOnline: async function (socket, data){
		try {
			console.log("PlayerOnline Room is called in regular  Omaha tournament ");
			...
			for(let i = 0; i< room.players.length; i++){
				if(room.players[i].id == data.playerId && room.players[i].status != 'Left'){
					...
					await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to(room.id).emit('onIdealPlayer', { 'playerId': room.players[i].id,status : false, roomId:room.id });
				}
			}
	...
	reconnectGame: async function (socket, data){
		try {
			...
			if(room.status == "Closed"){
				...
				let tournament = Sys.Game.Reguler.Omaha.Services.TournamentServices.getById(room.tournament);
				if(tournament.status == "Finished"){
					...
					let newRooms = await Sys.Game.Reguler.Omaha.Services.RoomServices.getByData({isTournamentTable: true, tournament: room.tournament, status:{ $ne: "Closed"}, players: { $elemMatch: { id: data.playerId, status: { $ne: "Left" } } }  });
					...
					if(newRooms.length > 0){
						...
						room = await Sys.Game.Reguler.Omaha.Services.RoomServices.get(newRooms[0].id);
					}else{
			...
			await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to(socket.id).emit('ResetGame', {roomId: room.id});
...
/*
	joinRoom: async function (socket, data){
		try {
			...
			var room = null;
			room = await Sys.Game.Reguler.Omaha.Controllers.RoomProcess.joinRoom(player, data);
			...
			if(room.game){
				...
				await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to([socket.id]).emit('OnSubscibePlayersCards', { playersCards : playersCards })
			}
	...
	leaveRoom: async function (socket, data) {
		try {
			//await Sys.Game.Reguler.Omaha.Services.playerGameHistoryServices.updatePlayerStatus(data.playerId,'Finished');
			return {
					status: 'success',
					message: 'Player Leave successfuly.',
					result: null
			};
			/*let responce = await Sys.Game.Reguler.Omaha.Controllers.RoomProcess.leftRoom(data);
	...
	sitOutNextHand: async function (socket, data){
		try {
			let responce = await Sys.Game.Reguler.Omaha.Controllers.RoomProcess.sitOutNextHand(data);
	...
	sitOutNextBigBlind: async function (socket, data){
		try {
			let responce = await Sys.Game.Reguler.Omaha.Controllers.RoomProcess.sitOutNextBigBlind(data);
			return { status: 'success', result: null, message: responce.message, statusCode: 200 }
		}
		catch (e) {
			console.log('Error in JoinRoom : ', e);
	...
	playerAddChips: async function (socket, data){
		try {
			...
			let playerUpdate = await Sys.Game.Reguler.Omaha.Services.PlayerServices.update(player.id, { chips: chips }); // Update Player Chips
	...
	showMyCards: async function (socket, data){
		try {
			if( data.gameId ){
				...
				/*let game = await Sys.Game.Reguler.Omaha.Services.GameServices.getSingleGameData({_id: data.gameId});
				if (!game || game == undefined) {
					return { 	status: 'fail',	result: null, message: "Game Not found!",statusCode: 404	};
				}
				let playerCards = ['BC','BC','BC','BC'];
				...
				await Sys.Io.of(Sys.Config.Namespace.CashRegularOmaha).to(room.id).emit('ShowCardResult', { playerId : data.playerId, cards:playerCards, roomId: room.id });
				return {
					status: 'success',
					result: {
						gameId : data.gameId ,
