...
class Room {
  constructor(id, tournament, tableType, currencyType, name, smallBlind, bigBlind,smallBlindIndex,bigBlindIndex, minPlayers, maxPlayers, minBuyIn, maxBuyIn, type, gameLimit, status, dealer, players, gameWinners, gameLosers, turnBet, game, currentPlayer, rackPercent, rackAmount, expireTime, owner, tableNumber,timerStart,limit,isTournamentTable,tournamentType,breakTime) {
  ...
  getAllHands() {
   ...
   AddPlayer(id, socketId, playerName, avatar, appid, chips, seatIndex, autoBuyin) {
    let room = this;
   // if (chips >= this.minBuyIn) { // Remove for Teting
      let player = new Sys.Game.Reguler.Texas.Entities.Player(id, socketId,seatIndex, playerName, avatar, appid, "Waiting", chips,false,false,false,[],autoBuyin,0,false,false,false,false);
     ...
     //  console.log("Befor create Game",gameobj);
      let game = await Sys.Game.Reguler.Texas.Services.GameServices.create(gameobj);
       ...
       if(game){
        ...
        // Update All Player Some Date not Status.
        ...
        let playersLength = 0;
         ...
         if (playersLength >= room.minPlayers) {
          ...
          console.log("No Minimum Player Found.... So Game not Starting....");
         }
       }else{ // Shiv!@#
