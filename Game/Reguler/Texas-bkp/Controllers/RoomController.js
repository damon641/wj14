...
module.exports = {
	...
	subscribeRoom: async function (socket, data){
		try {
			...
			if(room.game){
				...
				await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(socket.id).emit('PlayerCards', data)
			}
	...
	reconnectGame: async function (socket, data){
		try {
			...
			await Sys.Io.of(Sys.Config.Namespace.CashRegularTexas).to(socket.id).emit('ResetGame', {});
