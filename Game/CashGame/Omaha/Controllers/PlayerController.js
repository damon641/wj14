...
module.exports = {
    ...
    tip: async function(socket,data){
       /* try {
          ...
          let player = await Sys.Game.CashGame.Omaha.Services.PlayerServices.getByData({ _id :socket.playerId })
          if (player) {
             var chips = player.chips - parseFloat(data.chips);
             let update = await Sys.Game.CashGame.Omaha.Services.PlayerServices.update(socket.playerId, {chips: chips} );
             // added by K@Y
  						let transactionData = {
  							user_id						:	player.id,
            ...
            await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(data.roomId).emit('Tip', data)
             return {
               status: 'success',
               result:  null,
    ...
    chat: async function(socket, data){
      try {
        await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(data.roomId).emit('Chat', data)
          return {
            status  : 'success',
            result  : null,
    ...
    gift: async function(socket, data){
      try {
        await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(data.roomId).emit('PlayerGift', data)
        return {
          status: 'success',
          result  : null,
    ...
    inviteFriend: async function(socket, data){
      try {
        let player = await Sys.Game.CashGame.Omaha.Services.PlayerServices.getById(data.friendId);
        if (!player) {
            return {
                status: 'fail',
                result: null,
        ...
        let sender = await Sys.Game.CashGame.Omaha.Services.PlayerServices.getById(data.playerId);
        if (!sender) {
            return {
                status: 'fail',
                result: null,
        ...
        let room = await Sys.Game.CashGame.Omaha.Services.RoomServices.get(gamePlayer.room);
        if (!room) {
            return {
                status: 'fail',
                result: null,
        ...
        await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(data.roomId).emit('InviteFriend', gamePlayer)
        return {
            status : 'success',
            result : 'ok',
    ...
    playerAction: async function(socket, data){
      try {
        var action = await Sys.Game.CashGame.Omaha.Controllers.RoomProcess.playerAction(data);
        return {
          status: 'success',
          result  : null,
          data: action,
    ...
    playerDetails: async function(socket, data){
      try {
        let player = await Sys.Game.CashGame.Omaha.Services.PlayerServices.getById(data.id);
        if (player) {
          return {
              status: 'success',
              result: data,
  ...
  removePlayerFromRooms:async function() {
    try{
      console.log("omaha remove player calles")
              ...
              for(let p =0; p < players.length; p++){
                ...
                if(playerTimeout != null){
                  ...
                  if( removeAt < moment() ){
                    ...
                    let dataPlayer = await Sys.Game.CashGame.Omaha.Services.PlayerServices.getById(room.players[p].id);
                    ...
                    if (dataPlayer && room.players[p].isAllinPlayersChipsAssigned == false) {
                      ...
                      var playerUpdate = await Sys.Game.CashGame.Omaha.Services.PlayerServices.update(room.players[p].id, { chips: chips });
                      // added by K@Y
                      let transactionData = {
                      ...
                      console.log("omaha 5 minutes left player transactionLeftData: ", transactionLeftData);
                      await Sys.Game.CashGame.Texas.Services.PlayerAllTransectionService.createTransaction(transactionLeftData);
                    }
                    let playerUpdated = await Sys.Game.CashGame.Omaha.Services.RoomServices.update(room)
            ...
            if(room.status != 'Running' && totalPlayingPlayers == 1){
              ...
              if(subscribePlayerTIme != null){
                ...
                if( removeAt < moment() ){
                  ...
                  let dataPlayer = await Sys.Game.CashGame.Omaha.Services.PlayerServices.getById(room.players[removePlayerIndex].id);
                  ...
                  if (dataPlayer && room.players[removePlayerIndex].isAllinPlayersChipsAssigned == false) {
                    ...
                    var playerUpdate = await Sys.Game.CashGame.Omaha.Services.PlayerServices.update(room.players[removePlayerIndex].id, { chips: chips });
                    ...
                    console.log("omaha idel Player left for game: ", transactionDataLeft);
      ...
      console.log("Error when ideal player auto remove error: ", error);
    }
  },
}
