...
module.exports = {
	...
	subscribeRoom: async function (socket, data){
		try {
			...
			var player = await Sys.Game.CashGame.Omaha.Services.PlayerServices.getById(data.playerId);
			let room = await Sys.Game.CashGame.Omaha.Controllers.RoomProcess.checkRoomSeatAvilability(socket,data);
			...
			if(room.game){
				...
				await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to([socket.id]).emit('OnSubscibePlayersCards', { playersCards : playersCards, roomId:room.id });
				...
				for(let i=0; i < room.players.length; i++){
					...
					if(room.players[i].id == data.playerId && room.players[i].status == 'Playing' && room.players[i].cards.length == 4 && room.players[i].folded == false){
						...
						await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to([socket.id]).emit('OnPlayerCards',{
							playerId : room.players[i].id,
			...
			await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to([socket.id]).emit('OnSubscribeRoom', {
				roomId : room.id,
		...
		try {
			...
			for(let i = 0; i< room.players.length; i++){
				if(room.players[i].id == data.playerId && room.players[i].status != 'Left'){
					...
					await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(room.id).emit('onIdealPlayer', { 'playerId': room.players[i].id,status : false, roomId:room.id });
	...
	reconnectGame: async function (socket, data){
		try {
			...
			await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(socket.id).emit('ResetGame', {roomId: room.id});
	...
	joinRoom: async function (socket, data){
		try {
			...
			if( (current_date >= m_start_date && current_date <= m_end_date && Sys.Setting.maintenance.status =='active') || Sys.Setting.maintenance.quickMaintenance == "active" ){
				...
				let player = await Sys.Game.CashGame.Omaha.Services.PlayerServices.getById(data.playerId);
				...
				let room = null;
				let testRoomPlayers = await Sys.Game.CashGame.Omaha.Services.RoomServices.get(data.roomId);
				let playersLength = 0;
				...
				room = await Sys.Game.CashGame.Omaha.Controllers.RoomProcess.joinRoom(player, data);
	...
	leaveRoom: async function (socket, data) {
		try {
			let responce = await Sys.Game.CashGame.Omaha.Controllers.RoomProcess.leftRoom(data);
			...
			if(responce.status == 'success'){
				...
				if(room && room.status == 'Running'){
					...
					if(totalPlayers <= 1){
						...
						await Sys.Game.CashGame.Omaha.Controllers.PlayerProcess.progress(room);
					}
				}*/
	...
	sitOutNextHand: async function (socket, data){
		try {
			let responce = await Sys.Game.CashGame.Omaha.Controllers.RoomProcess.sitOutNextHand(data);
	...
	sitOutNextBigBlind: async function (socket, data){
		try {
			let responce = await Sys.Game.CashGame.Omaha.Controllers.RoomProcess.sitOutNextBigBlind(data);
			return { status: 'success', result: null, message: responce.message, statusCode: 200 }
		}
		catch (e) {
			console.log('Error in JoinRoom : ', e);
	...
	playerAddChips: async function (socket, data){
		try {
			...
			let playerUpdate = await Sys.Game.CashGame.Omaha.Services.PlayerServices.update(player.id, { chips: chips }); // Update Player Chips
			...
			let transactionExtraChipsData = {
	      	...
	      	console.log("cash omaha add extra chips transactionExtraChipsData: ", transactionExtraChipsData);
	...
	showMyCards: async function (socket, data){
		try {
			if( data.gameId ){
				...
				/*let game = await Sys.Game.CashGame.Omaha.Services.GameServices.getSingleGameData({_id: data.gameId});
				...
				await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(room.id).emit('ShowCardResult', { playerId : data.playerId, cards:playerCards, roomId: room.id });
				return {
					status: 'success',
					result: {
						gameId : data.gameId ,
	...
	useTimeBank: async function (socket, data){
		try {
			console.log("omaha useTimeBank is called",data);
			...
			for(let i=0; i < room.players.length; i++ ){
				if(room.players[i].id == data.playerId){
					...
					await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(room.id).emit('OnTimeBankUse', {
						playerId: data.playerId,
						roomId: room.id
					});
					break;
