...
module.exports = {
    ...
    try {
      ...
      if (room.game) {
        ...
        /*for (let j = 0; j < room.players.length; j += 1) {
            if ( room.players[j].status === 'Ideal' ) {
                  ...
                  if(resultInMinutes >= 2){
                    ...
                    await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(room.id).emit('PlayerLeft', { 'playerId': room.players[j].id, roomId:room.id });
                    let playerInfoDummy = [];
                    ...
                    await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to([room.players[j].socketId]).emit('playerInfoList', {
        ...
        if (await self.checkForEndOfRound(room) === true || room.game.status == 'ForceFinishedFolded' || room.game.status == 'ForceFinishedAllIn') {
          ...
          if (room.game.status == 'ForceFinishedAllIn') {
            ...
            switch (room.game.roundName) {
              case 'Preflop':
                ...
                Sys.Timers[room.id] = setTimeout(async function (room) {
                  ...
                  Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(room.id).emit('RoundComplete', {
                    roundStarted: room.game.roundName,
                    // cards: room.game.board, // shubh
                    cards: tempBoardThree, // shubh
                    potAmount: room.game.pot,
                  ...
                  Sys.Timers[room.id] = setTimeout(function (room) {
                    ...
                    Sys.Timers[room.id] = setTimeout(async function (room) {
                      ...
                      for (j = 0; j < room.players.length; j += 1) {
                        hand = new Sys.Game.CashGame.Omaha.Entities.Hand(
                      ...
                      await Sys.Game.CashGame.Omaha.Controllers.RoomProcess.gameFinished(room, sidePot)
            ...
            /*if(room.game.status == 'ForceFinishedFolded'){
              ...
              if(room.otherData.lastFoldedPlayerId){
                 Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(room.id).emit('DisplayShowCardButton', { playerIdList : [room.otherData.lastFoldedPlayerId], gameId: room.game.id, roomId: room.id });
              }
            }*/
            ...
            await Sys.Game.CashGame.Omaha.Controllers.RoomProcess.roundFinished(room, sidePot)
          ...
          await Sys.Game.CashGame.Omaha.Controllers.RoomProcess.turnFinished(room);
        }
      }
    } catch (e) {
      console.log("Error: ", e);
    ...
    try {
      ...
      if(room.limit == "Hi-Lo" || room.limit == "Hi-Lo-pot_limit" || room.limit == "Hi-Lo-limit"){
        for(let h = 0; h < room.game.gamePot.length; h += 1){
          ...
          winAmountHighPart = + parseFloat( parseFloat(winTotalAmount) / 2 ).toFixed(2);
          ...
          for(let k = 0; k < hiWinners.length; k++){
            ...
            console.log("WIN PLAYER",winplr,winTotalAmount,winAmountHighPart,winAmountLowPart)
            ...
            if(k == 0){
              winAmount = parseFloat(parseFloat(winAmount)).toFixed(4);
            }
        ...
        for (let h = 0; h < room.game.gamePot.length; h += 1) {
          console.log("ROOOM SIDE POT",room.game.gamePot[h])
          let playerIndexes = room.game.gamePot[h].playerIndex;
  ...
  //       //     let players = await Sys.Game.CashGame.Omaha.Services.PlayerServices.getByIds(ids);
  ...
  //       //                         await Sys.Game.CashGame.Omaha.Services.PlayerServices.update({id: player.id}, player);
  ...
  //       //                         await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(room.id).emit('PlayerInfo', tempdata)
  ...
  //   try {
  //     let hand = await new Sys.Game.CashGame.Omaha.Entities.Hand(player.cards, board);
  ...
  //           let rankKicker = await Sys.Game.CashGame.Omaha.Controllers.PlayerProcess.rankKickers(ranks.replace(cards[i], ''), 1);
  ...
  //       if(rank === 128){
  //         let rankKicker = await Sys.Game.CashGame.Omaha.Controllers.PlayerProcess.rankKickers(ranks, 5);
  ...
  //         if (ranks.indexOf(cards[i]) > -1 && rank === 0) {
  //           let rankKicker = await Sys.Game.CashGame.Omaha.Controllers.PlayerProcess.rankKickers(ranks.replace(cards[i], ''), 2);
  //           rank = rnk + rankKicker.rank; setBestCard([cards[i], rankKicker.cards]);
  //         }
  //         rnk--;
  //       }
  ...
  //               let rankKicker = await Sys.Game.CashGame.Omaha.Controllers.PlayerProcess.rankKickers(ranks.replace(cards[i], '').replace(cards[k], ''), 1);
  ...
  //       for (let i = 0; i < cards.length; i++) {
  //         if (ranks.indexOf(cards[i]) > -1 && rank === 0) {
  //           let rankKicker = await Sys.Game.CashGame.Omaha.Controllers.PlayerProcess.rankKickers(ranks.replace(cards[i], ''), 3);
  //           rank = rnk + rankKicker.rank; setBestCard([cards[i], rankKicker.cards]);
  //         }
  //         rnk--;
  //       }
  ...
  //         if (ranks.indexOf(cards[i]) > -1 && rank === 0) {
  //   //        console.log(">>>>>>>>>>>>>>>>>>>>>> :",cards[i]);
  //           let rankKicker = await Sys.Game.CashGame.Omaha.Controllers.PlayerProcess.rankKickers(ranks.replace(cards[i], ''), 4);
  //    //       console.log("Ranks >> :",ranks);
      ...
      if (room.game.gameRevertPoint[h].amount > 0 && isTransferValidated == true) {
        ...
        for (let w = 0; w < room.players.length; w += 1) {           
          if(room.players[w].id == room.game.gameRevertPoint[h].playerID ){
            ...
            console.log("revertPoint totalBetAmount: ", totalBetAmount);
        ...
        if (isFolded == true) {
          ...
          if (dataObj.forcefinishfolded == true && room.currencyType != 'chips') {
            ...
            newRoom = await Sys.Game.CashGame.Omaha.Controllers.RoomProcess.rackDeduction(room);
            ...
            await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(newRoom.id).emit('RevertPointFolded', dataObj);
            ...
            room.game.gameTotalChips= parseFloat(parseFloat(room.game.gameTotalChips)-parseFloat(dataObj.amount)).toFixed(4);
                        await Sys.Io.of(Sys.Config.Namespace.CashOmaha).to(room.id).emit('RevertPointFolded', dataObj);
          }
