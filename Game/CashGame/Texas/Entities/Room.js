...
class Room {
  ...
  AddPlayer(id, socketId, playerName, avatar, fb_avatar, chips, seatIndex, autoBuyin, subscribeTime,longitude,latitude,waitForBigBlindCheckbox,waitForBigBlindCheckboxValue,uniqId,sessionId,timeBank,timeBankDate) {
    ...
    if (chips >= room.minBuyIn) {
      let player = new Sys.Game.CashGame.Texas.Entities.Player(id, socketId,seatIndex, playerName, avatar, fb_avatar, "Waiting", chips,0,chips,false,false,false,[],autoBuyin,0,false,false,false,false,false,null,null, subscribeTime, false, false,false, 0,false, false, false, false, longitude,latitude,0,waitForBigBlindCheckbox,waitForBigBlindCheckboxValue,false,uniqId,sessionId,false,timeBank, false, timeBankDate);
  ...
  waitingAddPlayer(id, socketId, playerName, avatar, fb_avatar, chips, seatIndex, autoBuyin, subscribeTime,longitude,latitude,waitForBigBlindCheckbox,waitForBigBlindCheckboxValue,uniqId,sessionId,timeBank,timeBankDate) {
    ...
    console.log("AddPlayer room.minBuyIn: ", room.minBuyIn);
    if (chips >= room.minBuyIn) {
      let waitingPlayer = new Sys.Game.CashGame.Texas.Entities.Player(id, socketId,seatIndex, playerName, avatar, fb_avatar, "Waiting", chips,0,chips,false,false,false,[],autoBuyin,0,false,false,false,false,false,null,null, subscribeTime, false, false,false, 0,false, false, false, false, longitude,latitude,0,waitForBigBlindCheckbox,waitForBigBlindCheckboxValue,false,uniqId,sessionId,false,timeBank, false, timeBankDate);
  ...
  async StartGame() {
    try {   let m_start_date = moment(new Date(Sys.Setting.maintenance.maintenance_start_date)).tz(Intl.DateTimeFormat().resolvedOptions().timeZone).format("YYYY-MM-DD HH:mm");
...
*/
       ...
       /* let transactionDataBB = {
        ...
        if (playersLength >= room.minPlayers) {
           room.game.status = 'Running';
           room.status = 'Running';
           let gameStarted = await Sys.Game.CashGame.Texas.Controllers.RoomProcess.newGameStarted(room);
           room.initNewRound();
        }else{
  ...
  async initNewRound() {
    ...
    for (let i = 0; i < room.players.length; i += 1) {
      let player = await Sys.Game.CashGame.Texas.Services.PlayerServices.getById(room.players[i].id);
  ...
  getCurrentTurnButtonAction() {
    ...
    console.log("Room Called",this.getPreviousPlayerAction());
    ...
    console.log("maxBetOnRaise in room", parseFloat( this.game.maxBetOnRaise),raisedAmount );
