...
const { roomInfo } = require('./RoomController');
...
module.exports = {
        ...
        try {
            ...
            if (seatAvailable) {
                ...
                if (oldPlayer) {
                    ...
                    console.log("ROOOM TIME BANK", room.timeBank)
                    console.log("CURRENT TIME", moment(new Date()).format("YYYY-MM-DD HH:mm"))
                    console.log("OLD TIME", moment(new Date(oldPlayer.timeBankDate)).format("YYYY-MM-DD HH:mm"))
                    console.log("ITS NEW DAY", moment(moment(new Date()).format("YYYY-MM-DD HH:mm")).diff(moment(new Date(oldPlayer.timeBankDate)).format("YYYY-MM-DD HH:mm"), 'days') > 0)
                ...
                let updatedPlayerChipsTemp = await Sys.Game.CashGame.Texas.Services.PlayerServices.getById(data.playerId);
    ...
    leftRoom: async function(data) {
        try {
            ...
            let waitingPlayerLeft = await Sys.Game.CashGame.Texas.Controllers.RoomProcess.leftRoomForWaitingPlayers(data);
                ...
                Sys.Timers[room.id] = setTimeout(async function(room) {
                    ...
                    if (room.getCurrentPlayer()) {
                        ...
                        if (totalPlayingPlayers > 1) {
                            ...
                            Sys.Timers[room.id] = setInterval(async function(room) {
                                try {
                                    console.log("Player Timer Send 2 : ", timer, room.game.gameNumber, room.getCurrentPlayer().isUseTimeBank);
                ...
                if (room.getCurrentPlayer()) {
                    Sys.Timers[room.id] = setTimeout(async function(room) {
                        ...
                        if (totalPlayingPlayers > 1) {
                            ...
                            let maxTime = 0;
                            Sys.Timers[room.id] = setInterval(async function(room) {
                                console.log("Player Timer Send 3 roundFinish: ", timer, room.game.gameNumber, room.getCurrentPlayer().isUseTimeBank);
                                ...
                                if (timer < 0) {
                                    ...
                                    if (room.players[room.currentPlayer].isAlreadyActed == false) {
                                        if (room.getCurrentPlayer().isUseTimeBank == true && extraTimerOn != true) {
                                            console.log("TIME BANK", room.getCurrentPlayer().timeBank, isRunningBank)
                                            if (isRunningBank == true) {
    ...
    gameFinished: async function(room, sidePot) {
        try {
            ...
            let extraWaitTime = 500; // before it was 1000 
            ...
            Sys.Timers[room.id] = setTimeout(async function(room) {
                ...
                for (var rp = 0; rp < room.players.length; rp++) {
                    ...
                    //}
                    room.players[rp].chips = +parseFloat(room.players[rp].chips).toFixed(4)
                }
                ...
                if (room.game.gameTotalChips <= -0.01 || room.game.gameTotalChips >= 0.01) {
                    ...
                    Sys.Game.CashGame.Texas.Services.ChipsServices.createErrorLog(errorLog);
                }
                ...
                Sys.Game.CashGame.Texas.Controllers.RoomProcess.saveGameToHistry(room);
                            ...
                            if (room.waitingPlayers[j] && room.waitingPlayers[j].status == 'Left') {
                                ...
                                if (dataPlayer && room.waitingPlayers[j].isAllinPlayersChipsAssigned == false && room.waitingPlayers[j].status == 'Left') {
                                    ...
                                    if (room.waitingPlayers[j].extraChips > 0) {
                                        console.log("extra chips of lefted player", room.waitingPlayers[j].id, room.waitingPlayers[j].extraChips);
    ...
    removeOldPlayersFromRoom: async function(room) {
        try {
            if (room.oldPlayers) {
                console.log("oldplayers length", room.oldPlayers.length)
                for (let i = 0; i < room.oldPlayers.length; i++) {
    ...
    rackDeductionUpdate: async function(roomId, roomName, playerId, gameId, won, rackPercent, totalRack, rackFrom, rackTo, rackFromId, rackToId, gameNumber) {
        try {
            console.log("roomId,roomName,playerId, gameId, won, rackPercent, totalRack, rackFrom, rackTo, rackFromId, rackToId, gameNumber");
            console.log(roomId, roomName, playerId, gameId, won, rackPercent, totalRack, rackFrom, rackTo, rackFromId, rackToId, gameNumber);
            ...
            if (rackTo == 'admin') {
                ...
                if (admin != null && admin != undefined) {
                    ...
                    // }
                    /*if(admin.rake_chips){
                    	 adminChips = parseFloat(admin.rake_chips) + parseFloat(totalRack);
                    }else{
                    	admin.rake_chips = 0.00;	
                    	 adminChips = parseFloat(admin.rake_chips) + parseFloat(totalRack);
                    }

                    console.log("Admin chips--", admin.chips);
                    console.log("Admin rake_chips chips--", admin.rake_chips);
                    console.log("adminChips", adminChips);*/
                    //await Sys.App.Services.UserServices.updateUserData({ _id: rackToId }, { temp_chips: parseFloat(adminChips).toFixed(4) })
                    ...
                    let updatedUser = await Sys.App.Services.UserServices.findOneAndUpdate({ _id: rackToId }, { $inc: { rake_chips: +totalRack.toFixed(4) } });
                    console.log("updatedUser in rake", updatedUser);
                    console.log("updated admin rake", totalRack);
                ...
                let updatedAgent = await Sys.App.Services.agentServices.findOneAndUpdate({ _id: rackToId }, { $inc: { rake_chips: +totalRack.toFixed(4) } });
                console.log("updatedAgent in rake", updatedAgent);
                console.log("updated agent rake", totalRack);
    ...
    //   	if(foldPlyCount == 0 || (foldPlyCount != (parseInt(room.players.length)-1))){
    ...
    //   						allAgentsRackArray.unshift( ( 100 - allAgentsRackArray.reduce((a, b) => parseInt(a) + parseInt(b), 0) ) );
    ...
    //   		/*var roomUpdate = await Sys.Game.CashGame.Texas.Services.RoomServices.update(room);
    ...
    /*rackDeduction: async function (room) {
    	try {
    			...
    			for (let i = 0; i < room.gameWinners.length; i++) {
    				...
    				if(winPlayerDetail != null){
    					...
    					if(transactionDetail != null){
    						...
    						await Sys.Game.CashGame.Texas.Services.ChipsServices.updateTransactionData({user_id : room.gameWinners[i].playerId, gameId : room.game.id},{afterBalance:parseFloat(newafterBalance).toFixed(4), chips:parseFloat(newChips).toFixed(4)});
    					}
    		...
    		if (totalRackOfTheGame > 0) {
    			...
    			if (room.gameWinners) {
    				let insertData = await Sys.Game.CashGame.Texas.Services.PlayerTransectionHistoryService.insertPlayerTransectionHistrory(
    					{
    						gameId: room.game.id,
    						gameNumber: room.game.gameNumber,
    ...
    // 			let capData = await Sys.Game.CashGame.Texas.Services.ChipsServices.getCapByData({stack: stack});
    ...
    rackDeduction: async function(room) {
        try {
            ...
            if (totalRackOfTheGame > 0) {
                ...
                var rakeDistribution = [];
                for (let rp = 0; rp < room.players.length; rp++) {
                    if ((room.players[rp].status === 'Playing') || (room.players[rp].folded == true && room.game.roundBets[rp] > 0) || (room.players[rp].considerLeftedPlayer == true && room.game.roundBets[rp] > 0) || (room.players[rp].status === 'Left' && room.game.roundBets[rp] > 0) || (room.players[rp].status === 'Ideal' && room.game.roundBets[rp] > 0) || (room.players[rp].isAllInLefted == true && room.game.roundBets[rp] > 0)) {
                        console.log("player name and roundbets", room.players[rp].playerName, room.game.roundBets[rp]);
        ...
        try {
            ...
            if (room && room.waitingPlayers && room.waitingPlayers.length > 0) {
                for (let i = 0; i < room.waitingPlayers.length; i++) {
                    if (room.waitingPlayers[i].id == data.playerId && room.waitingPlayers[i].status != 'Left' && room.waitingPlayers[i].allIn == false) {
                        ...
                        leftedPlayerIndexId = room.waitingPlayers[i].id;
            ...
            if (player) {
                ...
                if (player.status != 'Left' && player.allIn == false) {
                    ...
                    for (i = 0; i < room.waitingPlayers.length; i++) {
                        if (room.waitingPlayers[i].id == leftedPlayerIndexId) {
                            room.waitingPlayers[i].status = "Left";
                            break;
                        }
                    }
                }
                ...
                let dataPlayer = await Sys.Game.CashGame.Texas.Services.PlayerServices.getById(player.id);
                if (dataPlayer && isAllinPlayerWhenLefted == false) {
                    for (t = 0; t < room.waitingPlayers.length; t++) {
                        if (room.waitingPlayers[t].id == player.id && room.waitingPlayers[t].isAllinPlayersChipsAssigned == false) {
                            console.log("Chips", dataPlayer.chips, player.chips, room.waitingPlayers[t].chips, player.extraChips, room.waitingPlayers[t].extraChips);
                            let chips = parseFloat(dataPlayer.chips) + parseFloat(room.waitingPlayers[t].chips) + parseFloat(room.waitingPlayers[t].extraChips);
                            console.log("Chips", dataPlayer.chips, room.waitingPlayers[t].chips, chips);

                            var playerUpdate = await Sys.Game.CashGame.Texas.Services.PlayerServices.update(player.id, { chips: chips, extraChips: 0 });

                            console.log("player update on chips assign", playerUpdate, player.id)
                            room.waitingPlayers[t].isAllinPlayersChipsAssigned = true;

                            console.log("----leftedPlayerIndexId-----", leftedPlayerIndexId);
                            room.waitingPlayers[t].extraChips = 0;

                            console.log("room.waitingPlayers[i].isAllinPlayersChipsAssigned in playerleft", room.waitingPlayers[t].isAllinPlayersChipsAssigned, room.waitingPlayers[t].playerName)
                            let updatedRoom = await Sys.Game.CashGame.Texas.Services.RoomServices.update(room);
                    ...
                    return { status: 'success', result: null, message: "Room Left Sccessd", statusCode: 401 };
                }
            } else {
                console.log(" No Player Found -----#### ");
                return { status: 'fail', result: null, message: "Player not found", statusCode: 401 };
