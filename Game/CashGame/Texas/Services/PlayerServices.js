...
module.exports = {
    ...
    update: async function(id,query){
        try {
            console.log("player update called:", id);
      ...
      findOneAndUpdate: async function(query, update, options){
        try {
            console.log("player update called in findOneAndUpdate:", query);
            let  player = await playerModel.findOneAndUpdate(query, update, options);
            return player;
