...
module.exports = {
    ...
    try {
      ...
      if (search != '') {
        ...
        let data = await Sys.App.Services.InAppPurchaseServices.getByData(query, null, {skip: start, limit: length,sort:{createdAt:-1}});
        var obj = {
          'draw': req.query.draw,
    ...
    editInAppPurchase:async function(req,res){
      try{
      let inAppPurchase = await Sys.App.Services.InAppPurchaseServices.getSingleData({_id: req.params.id}); 
    ...
    deleteInAppPurchase: async function(req, res){
        try {
            let inAppPurchase = await Sys.App.Services.InAppPurchaseServices.getSingleData({_id: req.body.id});
